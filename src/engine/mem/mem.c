...
static word mmChunkSizes[MM_MAX_NUM_CHUNKS];
static ChunkType mmChunkTypes[MM_MAX_NUM_CHUNKS];

static byte *mmRawMem;
static dword mmMemTotal;
static dword mmMemUsed;
static word mmChunksUsed;
...
void *MM_PushChunk(word size, ChunkType type) {
	...
	if (mmMemUsed + size > mmMemTotal) {
		sprintf(engine.system_error_message1, "MM_PushChunk function error");
		sprintf(engine.system_error_message2, "Error trying to get %u bytes", size);
		sprintf(engine.system_error_message3, "Total memory available: %ld, Used: %ld", mmMemTotal, mmMemUsed);
		Error(engine.system_error_message1, engine.system_error_message2, engine.system_error_message3, ERROR_MEMORY);
	}
	...
	if (mmChunksUsed >= MM_MAX_NUM_CHUNKS) {
		sprintf(engine.system_error_message1, "MM_PushChunk function error");
		sprintf(engine.system_error_message2, "Maximum number of chuncks used. Max %d", mmChunksUsed);
		sprintf(engine.system_error_message3, "");
		Error(engine.system_error_message1, engine.system_error_message2, engine.system_error_message3, ERROR_MEMORY);
		Error("No available memory chunks", "", "", ERROR_SYSTEM);
	}
